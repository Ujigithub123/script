...
-- =========================================================
-- HARVEST & PLANT - COLLECT DROP + CEK SEED
-- =========================================================
local function harvestAndPlantTile(tile)
    ...
    -- Collect drop setelah harvest
    print("[PNB] [HARVEST] Collecting drops di " .. coordStr)
    collectAtPoint(tile:point())

    -- Cek seed setelah collect, sebelum plant
...
local function pnbPlaceAndBreak()
    ...
    -- Phase 2: Collect di setiap titik target (kiri dan kanan tile)
    print("[PNB] PHASE 2: Collecting at each target tile...")
    sleep(math.random(800, 1200))

    -- Collect di startPos dulu
    collectAtPoint(startPos)
    sleep(math.random(200, 350))

    -- Collect di setiap offset target agar tile kiri & kanan ter-collect
    for i = 1, #PNB_CONFIG.offsets do
        local offset = PNB_CONFIG.offsets[i]
        local target = Vector2i.new(startPos.x + offset.x, startPos.y + offset.y)
        if PNB_CONFIG.log then
            print(string.format("[PNB] [COLLECT] At offset %d/%d: (%d,%d)", i, #PNB_CONFIG.offsets, target.x, target.y))
        end
        collectAtPoint(target)
        sleep(math.random(150, 250))
    end

    -- Collect ulang di startPos untuk catch item terlambat
...
-- =========================================================
-- 7) RECONNECT / WARP - VERSI SIMPEL ORIGINAL
-- =========================================================
